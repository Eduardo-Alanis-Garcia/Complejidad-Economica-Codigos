...
DNUE_Localidad = function(direccion = "2024A/", descomprimido = "2024A/Descomprimidos/", guardar = "2024A.csv"){
  ...
  colm = c("cve_ent", "cve_mun", "cve_loc","codigo_act", "nombre_act", "per_ocu")
...
for (i in 1:length(d)) {
  prueba = DNUE_Localidad(direccion = d[i], descomprimido = des[i] , guardar = g[i])
}
...
DNUE_Localidad = function(direccion = "2024A/", descomprimido = "2024A/Descomprimidos/", guardar = "2024A.csv"){
  ...
  library(dplyr)
  resumen = union |>
    group_by(clave.municipio, clave.localidad, cã.digo.de.la.clase.de.actividad.scian, descripcion.estrato.personal.ocupado) |>
    summarise(conteo = n())
...
prueba = DNUE_Localidad(direccion = "2015/" , descomprimido = "2015/Descomprimidos/" , guardar = "2015.csv")
